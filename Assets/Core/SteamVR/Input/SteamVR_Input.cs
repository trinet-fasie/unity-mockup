...
namespace Valve.VR
{
    public partial class SteamVR_Input
    {
        ...
        public static void IdentifyActionsFile(bool showLogs = true)
        {
            ...
            #if UNITY_EDITOR
            actionsPath = SteamVR_Settings.instance.actionsFilePath;
            #else
            actionsPath = "actions.json";
            #endif
            
            string fullPath = System.IO.Path.Combine(currentPath, actionsPath);
            ...
            if (File.Exists(fullPath))
            {
                ...
                if (err != EVRInputError.None)
                    Debug.LogWarning("<b>[SteamVR]</b> Error loading action manifest into SteamVR: " + err.ToString());
                else
                {
                    int numActions = 0;
