...
namespace VRTK
{
    ...
    public class VRTK_CustomRaycast : MonoBehaviour
    {
        ...
        public virtual bool CustomRaycast(Ray ray, out RaycastHit hitData, float length = Mathf.Infinity)
        {
            ...
            LayerMask customIgnoreLayers = ~layersToIgnore;

            int UILayer = LayerMask.NameToLayer("UI");
            
            if (MenuMode || PopupWindowManager.PopupButtonIsEnabled)
            {
                customIgnoreLayers = 1 << UILayer;
            }
            
            return Physics.Raycast(ray, out hitData, length, customIgnoreLayers, triggerInteraction);
